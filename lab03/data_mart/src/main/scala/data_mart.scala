...
object data_mart extends App {
  ...
  val password = fromFile("conf").getLines.toList(0)
  ...
  val webLogDF_cat = webLogDF
    .join(broadcast(siteCatsDF), Seq("domain"), "inner")
    .groupBy(col("uid"))
  ...
  val jdbcUrl = "jdbc:postgresql://10.0.1.9:5432/vladislav_leonov?user=vladislav_leonov&password=tsy6giqK"
